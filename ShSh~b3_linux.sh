...
rm -rf resources
mv ~/Desktop/ShSh~b3_linux.sh ~/Desktop/ShShb3
cd ~/Desktop/ShShb3
rm *_macos
rm *.txt
rm *.zip
...
read -r -p "Would You Like To Open the Webpage? [y/N] " response
	...
	then
links https://www.theiphonewiki.com/wiki/Models
...
read board
echo "Fetching Blobs.."
./tsschecker_linux -d "$model" --boardconfig "$board" -s -l -e "$ecid" --nocache
...
read manifest
echo "Fetching Blobs.."
./tsschecker_linux -d $model1 --boardconfig $board1 -m $manifest -s -l -e $ecid --nocache
mv *shsh2 ~/Desktop/ShShb3/ShShBlobs
echo "SHSH BLOBS Have Been Moved To ShShBlobs"
echo "Thanks!"
elif [ "$a" = "4" ]
...
./img4tool_linux -s $shsh -v $manifest1
