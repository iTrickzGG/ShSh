...
read model
	...
	echo "!!!If You Have A 6S OR 6S PLUS You Need To Download This App On Your iDevice To Find Out: https://itunes.apple.com/app/battery-memory-system-status/id497937231?mt=8"
	echo "To Find Out, Search Your Device For This Page : https://www.theiphonewiki.com/wiki/Models"
read boardconfig
	echo "Almost There.."
	echo "[*] What Is Your ECID?"
	echo "To Find Out, Read The Instructions Below.."
	echo "Connect your iDevice to your computer."
	echo "Open iTunes, Navigate To The "Summary" section. At the top of the panel, click twice on the line that says <SerialNumber>"
	echo "Your ECID is displayed in hexadecimal Form"
read ecid
	echo "OK..This Looks Like An ECID To Me..I Guess"
	echo "We Are All Set!"
cd ~/Desktop
sleep 1
read -r -p "One Last Question..Do You Want To Specify An Apnonce [y/N] " response
if [[ $response =~ ^([yY][eE][sS]|[y/Y])$ ]]
    then
	echo "Type It Write Here!"
read nonce
echo "Fetching SHSH..."
chmod +x tsschecker_macos
./tsschecker_macos -d "$model" --boardconfig "$boardconfig" -s -l -e "$ecid1" --apnonce "$nonce" --nocache
...
read model
echo "[*] What Board Configuration Do You Have? <EX : n71map>"
echo "!!!If You Have A 6S OR 6S PLUS You Need To Download This App On Your iDevice To Find Out: https://itunes.apple.com/us/app/lirum-device-info-lite-system/id591660734?mt=8"
echo "To Find Out, Search Your Device For This Page : https://www.theiphonewiki.com/wiki/Models"
read boardconfig
echo "Almost There.."
echo "[*] What Is Your ECID?"
...
cd ~/Desktop
chmod +x tsschecker_macos
./tsschecker_macos -d "$model" --boardconfig "$boardconfig" -m "$manifest" -s -e "$ecid1" --apnonce "$nonce" --nocache
echo "Done!"
echo "Please Verify Your SHSH Blobs, Just In Case"
sleep 3
    else
...
sleep 1
read -r -p "Do You See <Rosi Tag = true & $board there? [y/N] " response
if [[ $response =~ ^([yY][eE][sS]|[yY])$ ]]
	then
echo "Great! Your SHSH Blobs Are Valid!"
echo "Done!"
     else
