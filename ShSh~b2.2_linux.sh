...
echo "To Setup This Script..Follow The Instructions Below"
	echo "Please Make Sure You Have "tsschecker_linux" On Desktop"
	echo "Please Make Sure You Have "img4tool_linux" On Desktop"
...
./tsschecker_linux -d "$model" --boardconfig "$boardconfig" -s -l -e "$ecid" --nocache
...
cd ~/Desktop
chmod +x tsschecker_linux
./tsschecker_linux -d "$model" --boardconfig "$boardconfig" -m "$manifest" -s -e "$ecid1" --nocache
echo "Done!"
echo "Please Verify Your SHSH Blobs, Just In Case"
echo "@iTrickzGG"
echo "Press Any Key To Dismiss"
...
chmod +x img4tool_linux
./img4tool_linux -s "$shshloc1" -v "$manifest1"
sleep 1
